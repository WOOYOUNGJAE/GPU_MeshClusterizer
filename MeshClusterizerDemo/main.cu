...
int main()
{
    ...
    thrust::for_each(thrust::device,
        ...
        [bvh_dev] __device__ (std::size_t idx) -> void {
    ...
    thrust::for_each(thrust::device,
        ...
        [bvh_dev] __device__ (const unsigned int idx) -> void {
            const auto self = bvh_dev.objects[idx];
    ...
    thrust::transform(
        thrust::make_counting_iterator<unsigned int>(0),
        thrust::make_counting_iterator<unsigned int>(N),
        random_points.begin(), [] __device__(const unsigned int idx) -> float4 {
    ...
    thrust::for_each(random_points.begin(), random_points.end(),
        [bvh_dev] __device__ (const float4 pos) -> void {
